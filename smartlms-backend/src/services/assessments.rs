...
pub async fn create_question_bank(
    ...
    assessment_db::create_question_bank(pool, user_id, req).await
...
pub async fn create_question(
    ...
    assessment_db::create_question(pool, req).await
        .map_err(|e| e.to_string())
}

pub async fn get_question(pool: &PgPool, question_id: Uuid) -> Result<Question, String> {
    assessment_db::get_question(pool, question_id).await
        .map_err(|e| e.to_string())?
...
pub async fn get_questions_in_bank(
    ...
    assessment_db::get_questions_in_bank(pool, bank_id).await
        .map_err(|e| e.to_string())
}
...
pub async fn create_assessment(
    ...
    assessment_db::create_assessment(pool, req).await
        .map_err(|e| e.to_string())
}
...
pub async fn get_assessment_detail(
    ...
    let assessment = assessment_db::get_assessment(pool, assessment_id).await
        .map_err(|e| e.to_string())?
        .ok_or("Assessment not found")?;
    
    let questions = assessment_db::get_assessment_questions(pool, assessment_id).await
        .map_err(|e| e.to_string())?;
    
    let attempt_count = assessment_db::count_attempts(pool, assessment_id).await
        .map_err(|e| e.to_string())?;
    
    let avg_score = assessment_db::avg_assessment_score(pool, assessment_id).await
        .map_err(|e| e.to_string())?;
...
pub async fn publish_assessment(
    ...
    assessment_db::publish_assessment(pool, assessment_id).await
        .map_err(|e| e.to_string())
}
...
pub async fn start_attempt(
    ...
    let attempt_count = assessment_db::count_user_attempts(pool, user_id, assessment_id).await
        .map_err(|e| e.to_string())?;
    ...
    assessment_db::create_attempt(pool, user_id, assessment_id).await
        .map_err(|e| e.to_string())
}
...
pub async fn submit_answer(
    ...
    let attempt = assessment_db::get_attempt(pool, attempt_id).await
        .map_err(|e| e.to_string())?
    ...
    let question = assessment_db::get_question(pool, req.question_id).await
        .map_err(|e| e.to_string())?
    ...
    let answer = Answer {
        ...
        points_earned: if is_correct { Some(question.points) } else { Some(0) },
    };
    
    assessment_db::save_answer(pool, &answer).await
        .map_err(|e| e.to_string())?;
    ...
    match question.question_type {
        QuestionType::MultipleChoice | QuestionType::TrueFalse => {
            if let Some(selected) = &req.selected_options {
                ...
                question.options.iter().any(|o| o.is_correct && selected.contains(&o.id))
            } else {
                false
            }
        }
...
pub async fn submit_attempt(
    ...
    // Calculate score
    let answers = assessment_db::get_attempt_answers(pool, attempt_id).await
        .map_err(|e| e.to_string())?;
    
    let total_points: i32 = answers.iter()
        .filter_map(|a| a.points_earned)
        .sum();
    
    // Get assessment for max points
    let assessment = assessment_db::get_assessment(pool, attempt.assessment_id).await
        .map_err(|e| e.to_string())?
        .ok_or("Assessment not found")?;
    
    // For now calculate from questions
    let questions = assessment_db::get_assessment_questions(pool, attempt.assessment_id).await
        .map_err(|e| e.to_string())?;
    ...
    // Update attempt
    let updated = assessment_db::complete_attempt(
        pool,
        attempt_id,
        score,
        passed,
...
pub async fn get_gradebook(
    ...
    let (grades, total) = assessment_db::get_gradebook(pool, course_id, user_id).await
        .map_err(|e| e.to_string())?;
...
pub async fn create_grade(
    ...
    assessment_db::create_grade(pool, &grade).await
        .map_err(|e| e.to_string())
}

fn get_letter_grade(percent: f32) -> String {
    if percent >= 90.0 { "A" }
    else if percent >= 80.0 { "B" }
    else if percent >= 70.0 { "C" }
    else if percent >= 60.0 { "D" }
    else { "F" }
    .to_string()
}