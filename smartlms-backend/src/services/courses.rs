...
use crate::models::course::{
    Course, CourseListResponse, CourseProgress, CourseStatus, CreateCourseRequest,
    CreateLessonRequest, CreateModuleRequest, Enrollment, Lesson, Module, ModuleWithLessons,
    UpdateCourseRequest,
};
use sqlx::PgPool;
use uuid::Uuid;
...
pub async fn create_course(
    ...
    course_db::create(pool, instructor_id, req)
        .await
...
pub async fn update_course(
    ...
    course_db::update(pool, course_id, req)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn get_course_detail(
    pool: &PgPool,
    course_id: Uuid,
) -> Result<CourseDetailResponse, String> {
    let course = course_db::find_by_id(pool, course_id)
        .await
        .map_err(|e| e.to_string())?
        .ok_or("Course not found")?;

    let modules = course_db::get_modules(pool, course_id)
        .await
        .map_err(|e| e.to_string())?;

    let mut modules_with_lessons = Vec::new();
    for module in modules {
        let lessons = course_db::get_lessons(pool, module.id)
            .await
            .map_err(|e| e.to_string())?;
...
/// Create module
pub async fn create_module(pool: &PgPool, req: &CreateModuleRequest) -> Result<Module, String> {
    course_db::create_module(pool, req)
        .await
        .map_err(|e| e.to_string())
}

/// Create lesson
pub async fn create_lesson(pool: &PgPool, req: &CreateLessonRequest) -> Result<Lesson, String> {
    // Validate
    if req.title.is_empty() {
        return Err("Lesson title required".to_string());
    }

    course_db::create_lesson(pool, req)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn enroll_user(
    ...
    if course_db::get_enrollment(pool, user_id, course_id)
        .await
        .map_err(|e| e.to_string())?
        .is_some()
    {
    ...
    course_db::create_enrollment(pool, user_id, course_id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn get_progress(
    ...
    course_db::get_progress(pool, user_id, course_id)
        .await
        .map_err(|e| e.to_string())?
...
pub async fn complete_lesson(
    ...
    let lesson = course_db::get_lesson_by_id(pool, lesson_id)
        .await
        .map_err(|e| e.to_string())?
        .ok_or("Lesson not found")?;

    let module = course_db::get_module_by_id(pool, lesson.module_id)
        .await
        .map_err(|e| e.to_string())?
        .ok_or("Module not found")?;

    // Update progress
    course_db::mark_lesson_complete(pool, user_id, module.course_id, lesson_id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn get_course_stats(pool: &PgPool, course_id: Uuid) -> Result<CourseStats, String> {
    ...
    let enrollment_count = course_db::count_enrollments(pool, course_id)
        .await
        .map_err(|e| e.to_string())?;

    let avg_progress = course_db::avg_progress(pool, course_id)
        .await
        .map_err(|e| e.to_string())?;

    let avg_rating = course_db::avg_rating(pool, course_id)
        .await
        .map_err(|e| e.to_string())?;
