...
pub mod service {
    ...
    pub async fn upgrade_institution(
        ...
        if !is_valid_upgrade_path(previous_tier, req.target_tier) {
            return Err(format!(
                ...
                previous_tier, req.target_tier
            ));
        }

        // If new license key provided, validate it
        let new_license = if let Some(ref key) = req.license_key {
            let status = license::validate_license(key).map_err(|e| e.to_string())?;
            ...
            if status.plan != req.target_tier {
                return Err(format!(
                    "License key is for {:?}, requested {:?}",
                    status.plan, req.target_tier
                ));
            }
        ...
        // Save to DB
        crate::db::institution::update_plan_tier(
            pool,
            institution_id,
            req.target_tier,
            new_license,
    ...
    pub async fn downgrade_institution(
        ...
        Ok(UpgradeResult {
            ...
            message: Some(format!(
                "Downgrade scheduled. Changes take effect in 30 days."
            )),
        })
    }
    ...
    pub fn compare_tiers(current: PlanTier, target: PlanTier) -> TierFeatureComparison {
        ...
        let features_gained: Vec<String> = target_features
            .iter()
            .filter(|f| !current_features.contains(f))
            .cloned()
            .collect();

        let features_lost: Vec<String> = current_features
            .iter()
        ...
        TierFeatureComparison {
            ...
            quota_changes: QuotaDiff {
                ...
                concurrent: (
                    current_quotas.max_concurrent_users,
                    target_quotas.max_concurrent_users,
                ),
            },
        }
    }
    ...
    pub fn check_feature_available(
        pool: &PgPool,
        institution_id: Uuid,
        feature: &str,
    ) -> Result<bool, String> {
        ...
        Ok(crate::services::license::FeatureMatrix::is_available(
            tier, feature,
        ))
    }
}
...
async fn log_downgrade_event(
    ...
    tracing::info!(
        "Institution {} scheduled downgrade from {:?} to {:?}. Reason: {:?}",
        institution_id,
        from,
        to,
        reason
