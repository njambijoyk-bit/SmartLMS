...
pub mod service {
    ...
    /// Get video by ID
    pub async fn get_video(pool: &PgPool, video_id: uuid::Uuid) -> Result<Option<Video>, String> {
        let row = sqlx::query!(
    ...
    pub async fn delete_video(pool: &PgPool, video_id: uuid::Uuid) -> Result<(), String> {
        sqlx::query!(
    ...
    pub async fn get_playback_url(pool: &PgPool, video_id: uuid::Uuid) -> Result<String, String> {
        let video = get_video(pool, video_id).await?.ok_or("Video not found")?;
