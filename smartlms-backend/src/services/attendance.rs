...
pub mod service {
    ...
    pub async fn generate_qr_code(
        ...
        let session = attendance_db::create_qr_session(pool, session_id, &code, expires, max_uses)
            .await
    ...
    pub async fn check_in_with_qr(
        ...
        // Find the QR session
        let qr_session = attendance_db::get_active_qr_by_code(pool, &req.code)
            .await
            .map_err(|e| e.to_string())?
        ...
        attendance_db::create_class_attendance(pool, &attendance)
            .await
            .map_err(|e| e.to_string())?;

        attendance_db::increment_qr_use_count(pool, qr_session.id)
            .await
            .map_err(|e| e.to_string())?;
    ...
    pub async fn get_attendance_report(
        ...
        let records = attendance_db::get_course_attendance(pool, course_id, start_date, end_date)
            .await
            .map_err(|e| e.to_string())?;
